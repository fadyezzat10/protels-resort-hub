...
function mergeCMSHotel(staticHotel: StaticHotel, cmsHotel: any): StaticHotel {
  return {
    ...
    mobile: cmsHotel.mobile || staticHotel.mobile,
    email: cmsHotel.email || staticHotel.email,
    emailReservations: cmsHotel.emailReservations || staticHotel.emailReservations,
    emailSales: cmsHotel.emailSales || staticHotel.emailSales,
    ...
    facilitySections: cmsHotel.facilitySections ?? staticHotel.facilitySections,
    allInclusivePlan: cmsHotel.allInclusivePlan ?? (staticHotel as any).allInclusivePlan,
  };
}
...
export function useMergedHotels(): { hotels: StaticHotel[]; isLoading: boolean } {
  ...
  const newCMSHotels = cmsHotels
    ...
    .map((ch: any): StaticHotel => ({
      ...
      mobile: ch.mobile,
      email: ch.email,
      emailReservations: ch.emailReservations,
      emailSales: ch.emailSales,
      mapEmbed: ch.mapEmbed,
      mapShareUrl: ch.mapShareUrl,
      ratings: ch.ratings,
      tripAdvisorRank: ch.tripAdvisorRank,
      facilitySections: ch.facilitySections,
      allInclusivePlan: ch.allInclusivePlan,
    }));
  ...
  if (cmsHotel && !staticHotel) {
    return {
      hotel: {
        ...
        mobile: cmsHotel.mobile,
        email: cmsHotel.email,
        emailReservations: cmsHotel.emailReservations,
        emailSales: cmsHotel.emailSales,
        mapEmbed: cmsHotel.mapEmbed,
        mapShareUrl: cmsHotel.mapShareUrl,
        address: cmsHotel.address,
        ratings: cmsHotel.ratings,
        tripAdvisorRank: cmsHotel.tripAdvisorRank ?? undefined,
        facilitySections: cmsHotel.facilitySections,
        allInclusivePlan: cmsHotel.allInclusivePlan,
      },
      isLoading,
    };
  }
