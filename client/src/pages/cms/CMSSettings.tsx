            ...
            <CardContent className="space-y-4">
              ...
              <div className="border-t pt-5 space-y-5">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3"><div><h5 className="text-sm font-medium">Hotels and booking links</h5><p className="text-xs text-gray-400">Use only real UpperBooking sitekeys supplied by Profitroom; the official Protels panel remains the primary booking flow.</p></div><Button type="button" variant="outline" size="sm" data-testid="button-add-profitroom-property" onClick={() => setProfitroomConfig((current) => ({ ...current, properties: [...current.properties, { id: "property-" + Date.now(), name: "New hotel", bookingUrl: "", siteKey: "", openMode: "site" }] }))}><Plus className="mr-1 h-4 w-4" /> Add hotel</Button></div>
                    <div className="space-y-3">
                      ...
                      <div key={property.id || index} className="grid gap-2 rounded-lg border bg-gray-50 p-3 sm:grid-cols-[1fr_1fr_160px_auto] sm:items-center">
                        ...
                        <Input data-testid={"input-profitroom-property-sitekey-" + index} value={property.siteKey} onChange={(e) => setProfitroomConfig((current) => ({ ...current, properties: current.properties.map((item, itemIndex) => itemIndex === index ? { ...item, siteKey: e.target.value } : item) }))} placeholder="Real sitekey only (optional)" />
